  ...
  function create(info: ts.server.PluginCreateInfo) {
    ...
    for (let k of Object.keys(
      ...
      (proxy as any)[k] = (...args: any[]) =>
        Function.prototype.apply.call(x, info.languageService, args);
    }

    // Transform .fnx files to virtual .tsx for TypeScript
    function getVirtualFileName(fileName: string): string {
    ...
    function parseFnxFile(content: string): string {
      const lines: string[] = [];

      // Add Fynix import with all common exports
      lines.push(
        `import { Fynix, nixState, nixStore, nixAsync, nixEffect, nixComputed, nixForm, type NixState, type NixStore, type NixAsyncResult, type NixFormState } from '@fynixorg/ui';`,
      );
      lines.push("");

      // Extract logic block
      const logicMatch = content.match(
    ...
    proxy.getCompletionsAtPosition = (fileName, position, options) => {
      ...
      const fynixCompletions: ts.CompletionEntry[] = [
        {
          name: "nixState",
    ...
    proxy.getCompletionEntryDetails = (
      ...
      if (fynixFunctions.includes(entryName)) {
        const fynixInfo: Record<string, any> = {
          nixState: {
            displayParts: [
    ...
    proxy.getQuickInfoAtPosition = (fileName, position) => {
      ...
      const fynixFunctionInfo: Record<string, any> = {
        nixState: {
          displayParts: [
            { text: "function ", kind: "keyword" },
            { text: "nixState", kind: "functionName" },
