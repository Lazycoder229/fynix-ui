...
function activate(context) {
    ...
    context.subscriptions.push(vscode.workspace.registerTextDocumentContentProvider("fynix-virtual", provider));
    ...
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand("fynix.createComponent", createComponent));
...
class FynixVirtualDocumentProvider {
    ...
    provideTextDocumentContent(uri) {
        ...
        const document = vscode.workspace.textDocuments.find((doc) => doc.uri.toString() === originalUri.toString());
        if (!document) {
 ...
 */
function transformFnxToTs(source) {
    const lines = [];
    // Parse logic block
 ...
 */
function provideHoverSupport(context) {
        ...
        provideHover(document, position, token) {
            ...
            const htmlElementDocs = {
                ...
                input: {
                    ...
                    attrs: "type, value, placeholder, r-input, r-change, disabled, required",
 ...
 */
function provideCompletionSupport(context) {
    const completionProvider = vscode.languages.registerCompletionItemProvider("fynix", {
        provideCompletionItems(document, position, token, context) {
                ...
                if (!beforeCursor.match(/<\w*$/) && !beforeCursor.endsWith("<")) {
                    const htmlElements = [
                        ...
                        },
                        {
                            name: "input",
                            desc: "Input field",
                            snippet: '<input type="${1:text}" placeholder="$2" r-input={(e) => {$3}}$4/>',
                        },
                        {
                            name: "a",
                            desc: "Anchor link",
                    ...
                    return htmlElements.map((el) => {
                        const item = new vscode.CompletionItem(el.name, vscode.CompletionItemKind.Snippet);
            ...
            if (linePrefix.endsWith("r-")) {
                ...
                return events.map((event) => {
                    const item = new vscode.CompletionItem(`r-${event}`, vscode.CompletionItemKind.Property);
                    item.insertText = new vscode.SnippetString(`r-${event}={(e) => \${1:/* handle ${event} */}}`);
                    item.documentation = new vscode.MarkdownString(`Fynix event handler for \`${event}\` events`);
                    return item;
                });
            }
 ...
 * Provide definition support
 */
function provideDefinitionSupport(context) {
    const definitionProvider = vscode.languages.registerDefinitionProvider("fynix", {
        provideDefinition(document, position, token) {
            const range = document.getWordRangeAtPosition(position);
            const word = document.getText(range);
            // Find variable definitions in logic block
            const text = document.getText();
            const logicMatch = text.match(/<logic\s+setup\s*=\s*["']?(ts|js)["']?\s*>([\s\S]*?)<\/logic>/i);
            if (logicMatch && logicMatch[2]) {
 ...
 */
function isInLogicBlock(document, position) {
    const text = document.getText();
 ...
 */
function isInViewBlock(document, position) {
    const text = document.getText();
    const offset = document.offsetAt(position);
...
function provideDiagnostics(context) {
    const diagnosticCollection = vscode.languages.createDiagnosticCollection("fynix");
...
function validateDocument(document, diagnosticCollection) {
    ...
    if (!/<view\s*>[\s\S]*?<\/view>/i.test(text)) {
        const diagnostic = new vscode.Diagnostic(new vscode.Range(0, 0, 0, 0), "Missing <view> block. Every .fnx file must have a <view> section.", vscode.DiagnosticSeverity.Error);
    ...
    if (logicMatch && !["ts", "js"].includes(logicMatch[1].toLowerCase())) {
        ...
        const endPos = document.positionAt(text.indexOf(logicMatch[0]) + logicMatch[0].length);
        const diagnostic = new vscode.Diagnostic(new vscode.Range(startPos, endPos), `Invalid setup attribute "${logicMatch[1]}". Must be "ts" or "js".`, vscode.DiagnosticSeverity.Error);
        diagnostic.source = "fynix";
        diagnostics.push(diagnostic);
    }
    // Check for TypeScript syntax in setup="js"
    if (logicMatch && logicMatch[1].toLowerCase() === "js") {
        const logicContent = text.match(/<logic\s+setup\s*=\s*["']?js["']?\s*>([\s\S]*?)<\/logic>/i);
        if (logicContent && logicContent[1]) {
            ...
            for (const { pattern, message } of tsPatterns) {
                if (pattern.test(logicContent[1])) {
                    ...
                    if (match) {
                        const startOffset = text.indexOf(logicContent[0]) + logicContent[0].indexOf(match[0]);
                        const startPos = document.positionAt(startOffset);
                        const endPos = document.positionAt(startOffset + match[0].length);
                        const diagnostic = new vscode.Diagnostic(new vscode.Range(startPos, endPos), `${message} detected with setup="js". Change to setup="ts" or remove TypeScript syntax.`, vscode.DiagnosticSeverity.Error);
                        diagnostic.source = "fynix";
                        diagnostics.push(diagnostic);
                    }
                }
